...
class InventoryController extends Controller
{
	...
	public function update_low_stock_notification(Request $request){
        ...
        $auth_result = $request->user()->hasPermission('edit_inventory');
        if($auth_result){
			Event::dispatch(new crmEvent($request->user(), " updated low stock notification for part with id " . $request['part_id']));
            $update_low_stock_notification_result = DB::table('repository_parts')->where('id', $request['part_id'])->update(array('low_stock_notification' => $request['low_stock_notification']));
            return $update_low_stock_notification_result;
        }
    }
